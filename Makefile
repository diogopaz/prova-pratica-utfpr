...
PROJECT_NAME=prova-pratica
STACK_NAME=prova-pratica-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://prova-pratica.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t collective/prova-pratica-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t collective/prova-pratica-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name prova-pratica-backend-acceptance -d collective/prova-pratica-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name prova-pratica-frontend-acceptance --link prova-pratica-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/prova-pratica-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop prova-pratica-frontend-acceptance
	@docker stop prova-pratica-backend-acceptance
